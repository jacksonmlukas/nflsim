...
class TestTimedeltaArraylikeMulDivOps:
    ...
    def test_td64arr_mul_tdlike_scalar_raises(self, two_hours, box_with_array):
        ...
        rng = tm.box_expected(rng, box_with_array)
        msg = "argument must be an integer|cannot use operands with types dtype|Cannot multiply with"
        with pytest.raises(TypeError, match=msg):
            rng * two_hours
    ...
    def test_td64arr_div_nat_invalid(self, box_with_array):
        ...
        msg = "ufunc '(true_divide|divide)' cannot use operands|cannot perform __r?truediv__|Cannot divide datetime64 by TimedeltaArray"
        with pytest.raises(TypeError, match=msg):
            rng / dt64nat
        with pytest.raises(TypeError, match=msg):
            dt64nat / rng
    ...
    def test_td64arr_div_numeric_array(
        ...
        pattern = "true_divide'? cannot use operands|cannot perform __div__|cannot perform __truediv__|unsupported operand|Cannot divide|ufunc 'divide' cannot use operands with types"
        with pytest.raises(TypeError, match=pattern):
            vector / tdser
