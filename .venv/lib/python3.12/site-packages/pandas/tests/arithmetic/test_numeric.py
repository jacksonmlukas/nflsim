...
class TestNumericArraylikeArithmeticWithDatetimeLike:
    ...
    def test_add_sub_datetimedeltalike_invalid(
        ...
        msg = "unsupported operand type|Addition/subtraction of integers and integer-arrays|Instead of adding/subtracting|cannot use operands with types dtype|Concatenation operation is not implemented for NumPy arrays|Cannot (add|subtract) NaT (to|from) ndarray|operand type\\(s\\) all returned NotImplemented from __array_ufunc__|can only perform ops with numeric values|cannot subtract DatetimeArray from ndarray|Cannot add or subtract Timedelta from integers"
...
class TestAdditionSubtraction:
    ...
    def test_series_frame_radd_bug(self, fixed_now_ts):
        ...
        msg = "unsupported operand type|Concatenation operation"
        with pytest.raises(TypeError, match=msg):
            fix_now + ts
