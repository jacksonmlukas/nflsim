...
class TestPeriodIndexArithmetic:
    ...
    def test_parr_add_sub_invalid(self, other, box_with_array):
        ...
        msg = "(:?cannot add PeriodArray and .*)|(:?cannot subtract .* from (:?a\\s)?.*)|(:?unsupported operand type\\(s\\) for \\+: .* and .*)|unsupported operand type\\(s\\) for [+-]: .* and .*"
    ...
    def test_parr_add_sub_timedeltalike_freq_mismatch_daily(
        ...
        msg = "Input has different freq(=.+)? from Period.*?\\(freq=D\\)|Cannot add/subtract timedelta-like from PeriodArray that is not an integer multiple of the PeriodArray's freq."
        with pytest.raises(IncompatibleFrequency, match=msg):
            rng + other
        with pytest.raises(IncompatibleFrequency, match=msg):
            rng += other
    ...
    def test_parr_add_timedeltalike_mismatched_freq_hourly(
        ...
        rng = period_range("2014-01-01 10:00", "2014-01-05 10:00", freq="h")
        rng = tm.box_expected(rng, box_with_array)
        msg = "Input has different freq(=.+)? from Period.*?\\(freq=h\\)|Cannot add/subtract timedelta-like from PeriodArray that is not an integer multiple of the PeriodArray's freq."
