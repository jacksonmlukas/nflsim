...
def assert_invalid_comparison(left, right, box):
    ...
    msg = "Invalid comparison between|Cannot compare type|not supported between|invalid type promotion|The DTypes <class 'numpy.dtype\\[datetime64\\]'> and <class 'numpy.dtype\\[int64\\]'> do not have a common DType. For example they cannot be stored in a single array unless the dtype is `object`."
    with pytest.raises(TypeError, match=msg):
        left < right
    with pytest.raises(TypeError, match=msg):
        left <= right
