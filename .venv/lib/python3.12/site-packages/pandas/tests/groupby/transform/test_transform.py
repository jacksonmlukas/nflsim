...
def test_cython_transform_frame_column(
    ...
    if (
        ...
        msg = "does not support .* operations|.* is not supported for object dtype|is not implemented for this dtype|.* is not supported for str dtype|dtype 'str' does not support operation '.*'"
        with pytest.raises(TypeError, match=msg):
            gb[c].transform(op)
        with pytest.raises(TypeError, match=msg):
            getattr(gb[c], op)()
