...
def test_deprecate_numeric_only_series(dtype, groupby_func, request):
    ...
    if groupby_func not in has_numeric_only:
        ...
        msg = "SeriesGroupBy.sem called with numeric_only=True and dtype object|Series.skew does not allow numeric_only=True with non-numeric|cum(sum|prod|min|max) is not supported for object dtype|Cannot use numeric_only=True with SeriesGroupBy\\..* and non-numeric"
        with pytest.raises(TypeError, match=msg):
            method(*args, numeric_only=True)
