...
class TestDataFrameAnalytics:
        ...
        if (opname in ("sum", "min", "max") and axis == 0) or opname in (
            ...
            if opname in ["var", "std", "sem", "skew", "kurt"]:
                ...
                if axis == 0:
                    ...
                    msg = "Could not convert \\['.*'\\] to numeric|Could not convert string '(bar){30}' to numeric"
                else:
                    msg = r"unsupported operand type\(s\) for \+: 'float' and 'str'"
    ...
    def test_mixed_ops(self, op):
        ...
        msg = "Could not convert|could not convert|can't multiply sequence by non-int|does not support|Cannot perform"
        with pytest.raises(TypeError, match=msg):
            getattr(df, op)()
...
def test_fails_on_non_numeric(kernel):
    ...
    msg = "not allowed for this dtype|argument must be a string or a number|not supported between instances of|unsupported operand type|argument must be a string or a real number"
    if kernel == "median":
