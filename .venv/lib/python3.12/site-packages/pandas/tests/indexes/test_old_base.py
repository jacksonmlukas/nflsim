...
class TestBase:
    ...
    def test_pickle_compat_construction(self, simple_index):
        ...
        msg = "Index\\(\\.\\.\\.\\) must be called with a collection of some kind, None was passed|DatetimeIndex\\(\\) must be called with a collection of some kind, None was passed|TimedeltaIndex\\(\\) must be called with a collection of some kind, None was passed|__new__\\(\\) missing 1 required positional argument: 'data'|__new__\\(\\) takes at least 2 arguments \\(1 given\\)"
    ...
    def test_insert_out_of_bounds(self, index, using_infer_string):
        ...
        msg = "index -?\\d+ is out of bounds for axis 0 with size \\d+|loc must be an integer between"
