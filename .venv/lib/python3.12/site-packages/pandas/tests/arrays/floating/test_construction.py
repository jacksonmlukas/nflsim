...
def test_to_array_error(values):
    ...
    msg = "cannot be converted to FloatingDtype|values must be a 1D list-like|Cannot pass scalar|float\\(\\) argument must be a string or a (real )?number, not 'dict'|could not convert string to float: 'foo'|could not convert string to float: np\\.str_\\('foo'\\)"
