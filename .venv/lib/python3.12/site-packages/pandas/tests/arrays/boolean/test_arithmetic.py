...
def test_error_invalid_values(data, all_arithmetic_operators):
    ...
    with pytest.raises(TypeError, match=msg):
        ops("foo")
    msg = "unsupported operand type\\(s\\) for|Concatenation operation is not implemented for NumPy arrays|has no kernel|not supported for dtype"
    with pytest.raises(TypeError, match=msg):
    ...
    if op not in ("__mul__", "__rmul__"):
        ...
        msg = "unsupported operand type\\(s\\) for|can only concatenate str|not all arguments converted during string formatting|has no kernel|not implemented|not supported for dtype"
        with pytest.raises(TypeError, match=msg):
