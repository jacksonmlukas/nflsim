...
class TestFancy:
    ...
    def test_setitem_ndarray_3d(self, index, frame_or_series, indexer_sli):
        ...
        if indexer_sli is tm.iloc:
            ...
            err = ValueError
            msg = "Buffer has wrong number of dimensions \\(expected 1, got 3\\)|Cannot set values with ndim > 1|Index data must be 1-dimensional|Data must be 1-dimensional|Array conditional must be same shape as self"
...
def test_scalar_setitem_with_nested_value(value):
    ...
    msg = "Must have equal len keys and value|setting an array element with a sequence"
    with pytest.raises(ValueError, match=msg):
        df.loc[0, "B"] = value
