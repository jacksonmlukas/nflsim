...
def array(
    *cols: Union[
        "ColumnOrName", Union[List["ColumnOrName"], Tuple["ColumnOrName", ...]]
    ]
) -> Column:
...
def slice(
    ...
    """
    start = (
        ConstantExpression(start) if isinstance(start, int) else _to_column_expr(start)
    )
    length = (
        ConstantExpression(length)
        if isinstance(length, int)
        else _to_column_expr(length)
    )
...
def levenshtein(
    ...
    if threshold is None:
        ...
        return Column(
            CaseExpression(
                distance <= ConstantExpression(threshold), distance
...
def lpad(col: "ColumnOrName", len: int, pad: str) -> Column:
    ...
    """
    return _invoke_function(
        "lpad", _to_column_expr(col), ConstantExpression(len), ConstantExpression(pad)
    )
...
def rpad(col: "ColumnOrName", len: int, pad: str) -> Column:
    ...
    """
    return _invoke_function(
        "rpad", _to_column_expr(col), ConstantExpression(len), ConstantExpression(pad)
    )
...
def asin(col: "ColumnOrName") -> Column:
    ...
    return Column(
        CaseExpression(
            (col < -1.0) | (col > 1.0), ConstantExpression(float("nan"))
        ).otherwise(FunctionExpression("asin", col))
    )
...
def like(
    ...
    return _invoke_function(
        "like_escape", _to_column_expr(str), _to_column_expr(pattern), escapeChar
    )
...
def ilike(
    ...
    if escapeChar is None:
        ...
        escapeChar = _to_column_expr(escapeChar)
    return _invoke_function(
        "ilike_escape", _to_column_expr(str), _to_column_expr(pattern), escapeChar
    )
...
def array_union(col1: "ColumnOrName", col2: "ColumnOrName") -> Column:
    ...
    return _invoke_function_over_columns(
        "array_distinct", _invoke_function_over_columns("array_concat", col1, col2)
    )
...
def array_max(col: "ColumnOrName") -> Column:
    ...
    return _invoke_function(
        "array_extract",
        _to_column_expr(_invoke_function_over_columns("array_sort", col)),
        _get_expr(-1),
    )
...
def approxCountDistinct(col: "ColumnOrName", rsd: Optional[float] = None) -> Column:
    ...
    """
    warnings.warn(
        "Deprecated in 2.1, use approx_count_distinct instead.", FutureWarning
    )
...
def array_compact(col: "ColumnOrName") -> Column:
    ...
    return _invoke_function(
        "list_filter",
        _to_column_expr(col),
        LambdaExpression("x", ColumnExpression("x").isnotnull()),
    )
...
def array_remove(col: "ColumnOrName", element: Any) -> Column:
    ...
    return _invoke_function(
        "list_filter",
        _to_column_expr(col),
        LambdaExpression("x", ColumnExpression("x") != ConstantExpression(element)),
    )
...
def char(col: "ColumnOrName") -> Column:
    ...
    col = _to_column_expr(col)
    return Column(
        FunctionExpression("chr", CaseExpression(col > 256, col % 256).otherwise(col))
    )
...
def pow(
    col1: Union["ColumnOrName", float], col2: Union["ColumnOrName", float]
) -> Column:
    """
...
def regexp_count(str: "ColumnOrName", regexp: "ColumnOrName") -> Column:
    ...
    """
    return _invoke_function_over_columns(
        "len", _invoke_function_over_columns("regexp_extract_all", str, regexp)
    )
...
def regexp_extract_all(
    str: "ColumnOrName",
    regexp: "ColumnOrName",
    idx: Optional[Union[int, Column]] = None,
) -> Column:
...
def regexp_substr(str: "ColumnOrName", regexp: "ColumnOrName") -> Column:
    ...
    return Column(
        FunctionExpression(
            "nullif",
            FunctionExpression(
                "regexp_extract", _to_column_expr(str), _to_column_expr(regexp)
            ),
            ConstantExpression(""),
        )
    )
        ...
        CaseExpression(
            ...
            CoalesceOperator(
                FunctionExpression(
                    "list_position",
                    FunctionExpression(
                        "string_split", str_array, ConstantExpression(",")
                    ),
                    str,
                ),
...
def atan2(
    ...
    return _invoke_function_over_columns(
        "atan2", lit_or_column(col1), lit_or_column(col2)
    )
...
def get(col: "ColumnOrName", index: Union["ColumnOrName", int]) -> Column:
    ...
    """
    index = (
        ConstantExpression(index) if isinstance(index, int) else _to_column_expr(index)
    )
...
def add_months(start: "ColumnOrName", months: Union["ColumnOrName", int]) -> Column:
    ...
    """
    months = (
        ConstantExpression(months)
        if isinstance(months, int)
        else _to_column_expr(months)
    )
    return _invoke_function(
        "date_add", _to_column_expr(start), FunctionExpression("to_months", months)
    ).cast("date")
    ...
    if null_replacement is not None:
        col = FunctionExpression(
            ...
            LambdaExpression(
                "x",
                CaseExpression(
                    ColumnExpression("x").isnull(), ConstantExpression(null_replacement)
                ).otherwise(ColumnExpression("x")),
            ),
        )
...
def array_position(col: "ColumnOrName", value: Any) -> Column:
    ...
    return Column(
        CoalesceOperator(
            _to_column_expr(
                _invoke_function_over_columns("list_position", col, lit(value))
            ),
            ConstantExpression(0),
        )
    )
...
def array_repeat(col: "ColumnOrName", count: Union["ColumnOrName", int]) -> Column:
    ...
    return _invoke_function_over_columns(
        "list_resize", _invoke_function_over_columns("list_value", col), count, col
    )
...
def array_sort(
    ...
    if comparator is not None:
        ...
        return _invoke_function_over_columns(
            "list_sort", col, lit("ASC"), lit("NULLS LAST")
        )
...
def split_part(
    src: "ColumnOrName", delimiter: "ColumnOrName", partNum: "ColumnOrName"
) -> Column:
    """
    ...
    return Column(
        CaseExpression(
            src.isnull() | delimiter.isnull() | partNum.isnull(),
            ConstantExpression(None),
        ).otherwise(
            CaseExpression(
                delimiter == ConstantExpression(""), ConstantExpression("")
            ).otherwise(part)
        )
    )
...
def _to_date_or_timestamp(
    col: "ColumnOrName", spark_datatype: _types.DataType, format: Optional[str] = None
) -> Column:
...
def arrays_overlap(a1: "ColumnOrName", a2: "ColumnOrName") -> Column:
    ...
    return Column(
        CaseExpression(
            ...
            CaseExpression(
                (FunctionExpression("len", a1) > 0)
                & (FunctionExpression("len", a2) > 0)
                & (a1_has_null | a2_has_null),
                ConstantExpression(None),
