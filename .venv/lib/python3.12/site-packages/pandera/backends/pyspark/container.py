...
class DataFrameSchemaBackend(PysparkSchemaBackend):
    ...
    def validate(
        ...
        *,
        ...
        check_obj = self._schema_checks(check_obj, schema, column_info, error_handler)

        # validate the rows (data) of the dataframe
        check_obj = self._data_checks(check_obj, schema, column_info, error_handler)
        ...
        sample = self.subsample(check_obj, sample=sample, random_state=random_state)
    ...
    def run_checks(self, check_obj: DataFrame, schema, error_handler):
        ...
        check_results = []
        for check_index, check in enumerate(schema.checks):  # schema.checks is null
            try:
    ...
    def unique(
        ...
        *,
        ...
        unique_columns = (
            [schema.unique] if isinstance(schema.unique, str) else schema.unique
        )
