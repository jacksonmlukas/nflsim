...
def _cached_search_compile(
    pattern: AnyStr, re_verbose: bool, re_version: bool, pattern_type: type[AnyStr]
) -> AnyStr:
...
def _cached_replace_compile(
    pattern: Pattern[AnyStr], repl: AnyStr, flags: int, pattern_type: type[AnyStr]
...
def _apply_replace_backrefs(
    m: Match[AnyStr] | None, repl: ReplaceTemplate[AnyStr] | AnyStr, flags: int = 0
) -> AnyStr:
    ...
    return (
        _bregex_parse._ReplaceParser(m.re, repl, bool(flags & FORMAT)).parse().expand(m)
    )


def _apply_search_backrefs(
    pattern: AnyStr | Pattern[AnyStr] | Bregex[AnyStr], flags: int = 0
    ...
    if isinstance(pattern, (str, bytes)):
        ...
        if not (flags & DEBUG):
            ...
            p = _bregex_parse._SearchParser(
                cast(AnyStr, pattern), re_verbose, re_version
            ).parse()
...
class Bregex(_util.Immutable, Generic[AnyStr]):
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        return (
            isinstance(other, Bregex)
            and self._pattern == other._pattern
            and self.auto_compile == other.auto_compile
        )
    ...
    def __ne__(self, other: Any) -> bool:
        ...
        return (
            not isinstance(other, Bregex)
            or self._pattern != other._pattern
            or self.auto_compile != other.auto_compile
        )
    ...
    def _auto_compile(
        self, template: AnyStr | Callable[..., AnyStr], use_format: bool = False
        ...
        if isinstance(template, ReplaceTemplate):
        ...
        elif isinstance(template, ReplaceTemplate) or (
            isinstance(template, (str, bytes)) and self.auto_compile
        ):
            return self.compile(
                template,
                (
                    FORMAT
                    if use_format and not isinstance(template, ReplaceTemplate)
                    else 0
                ),
            )
            ...
            raise AttributeError(
                "Format replaces cannot be called without compiling replace template!"
            )
        return template

    def compile(  # noqa A001
        self, repl: AnyStr | Callable[..., AnyStr], flags: int = 0
    ) -> Callable[..., AnyStr]:
        """Compile replace."""
    ...
    def search(self, string: AnyStr, *args: Any, **kwargs: Any) -> Match[AnyStr] | None:
        """Apply `search`."""

        return self._pattern.search(string, *args, **kwargs)

    def match(self, string: AnyStr, *args: Any, **kwargs: Any) -> Match[AnyStr] | None:
        """Apply `match`."""

        return cast(
            "Match[AnyStr] | None", self._pattern.match(string, *args, **kwargs)
        )

    def fullmatch(
        self, string: AnyStr, *args: Any, **kwargs: Any
    ) -> Match[AnyStr] | None:
        """Apply `fullmatch`."""

        return cast(
            "Match[AnyStr] | None", self._pattern.fullmatch(string, *args, **kwargs)
        )
    ...
    def findall(
        ...
        return cast(
            "list[AnyStr] | list[tuple[AnyStr, ...]]",
            self._pattern.findall(string, *args, **kwargs),
        )
    ...
    def finditer(
        ...
        return cast(
            Iterator[Match[AnyStr]], self._pattern.finditer(string, *args, **kwargs)
        )
    ...
    def sub(
        ...
        **kwargs: Any,
        ...
        return cast(
            AnyStr, self._pattern.sub(self._auto_compile(repl), string, *args, **kwargs)
        )
    ...
    def subf(
        ...
        **kwargs: Any,
        ...
        return cast(
            AnyStr,
            self._pattern.subf(self._auto_compile(repl, True), string, *args, **kwargs),
        )
    ...
    def subn(
        ...
        **kwargs: Any,
        ...
        return cast(
            "tuple[AnyStr, int]",
            self._pattern.subn(self._auto_compile(repl), string, *args, **kwargs),
        )
    ...
    def subfn(
        ...
        **kwargs: Any,
        ...
        return cast(
            "tuple[AnyStr, int]",
            self._pattern.subfn(
                self._auto_compile(repl, True), string, *args, **kwargs
            ),
        )
...
def compile_search(
    pattern: AnyStr | Pattern[AnyStr] | Bregex[AnyStr], flags: int = 0, **kwargs: Any
) -> Pattern[AnyStr]:
    """Compile with extended search references."""

    return cast(
        Pattern[AnyStr],
        _regex.compile(_apply_search_backrefs(pattern, flags), flags, **kwargs),
    )


def compile_replace(
    pattern: Pattern[AnyStr], repl: AnyStr | Callable[..., AnyStr], flags: int = 0
) -> Callable[..., AnyStr]:
    ...
    if pattern is not None and isinstance(pattern, _REGEX_TYPE):
        if isinstance(repl, (str, bytes)):
            if not (pattern.flags & DEBUG):
                ...
                call = _bregex_parse._ReplaceParser(
                    pattern, repl, bool(flags & FORMAT)
                ).parse()
        elif isinstance(repl, ReplaceTemplate):
            if flags:
                raise ValueError(
                    "Cannot process flags argument with a ReplaceTemplate!"
                )
...
def match(
    ...
    **kwargs: Any,
    ...
    return cast(
        "Match[AnyStr] | None",
        _regex.match(
            _apply_search_backrefs(pattern, flags), string, flags, *args, **kwargs
        ),
    )
    ...
    **kwargs: Any,
    ...
    return cast(
        ...
        _regex.sub(
            pattern,
            (
                compile_replace(pattern, repl, flags=rflags)
                if is_replace or is_string
                else repl
            ),
            string,
            count,
...
def finditer(
    ...
    **kwargs: Any,
    ...
    return cast(
        Iterator[Match[AnyStr]],
        _regex.finditer(
            _apply_search_backrefs(pattern, flags), string, *args, **kwargs
        ),
    )
