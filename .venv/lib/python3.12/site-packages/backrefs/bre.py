...
def _cached_search_compile(
    pattern: AnyStr, re_verbose: bool, re_unicode: bool, pattern_type: type[AnyStr]
) -> AnyStr:
...
def _apply_search_backrefs(
    pattern: AnyStr | Pattern[AnyStr] | Bre[AnyStr], flags: int = 0
    ...
    if pattern is not None and isinstance(pattern, _RE_TYPE):
        if isinstance(repl, (str, bytes)):
            if not (pattern.flags & DEBUG):
                ...
                call = _bre_parse._ReplaceParser(
...
def match(
    ...
    **kwargs: Any,
    ...
    return _re.match(
        _apply_search_backrefs(pattern, flags), string, flags=flags, **kwargs
    )
