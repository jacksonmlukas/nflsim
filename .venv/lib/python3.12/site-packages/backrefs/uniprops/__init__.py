...
def get_gc_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_properties if mode != MODE_UNICODE else unidata.unicode_properties
    )
    ...
    if not negate:
        ...
        value = (
            "".join(
                [
                    fmt_string(v, is_binary)
                    for k, v in obj.get(p1, {}).items()
                    if not k.startswith("^")
                ]
            )
            if p2 is None
            else fmt_string(obj.get(p1, {}).get(p2, ""), is_binary)
        )
    else:
...
def get_canonical_combining_class_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_canonical_combining_class
        if mode != MODE_UNICODE
        else unidata.unicode_canonical_combining_class
    )

    if value.startswith("^"):
        negated = value[1:]
        value = "^" + unidata.unicode_alias["canonicalcombiningclass"].get(
            negated, negated
        )
    else:
...
def get_east_asian_width_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_east_asian_width
        if mode != MODE_UNICODE
        else unidata.unicode_east_asian_width
    )
...
def get_grapheme_cluster_break_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_grapheme_cluster_break
        if mode != MODE_UNICODE
        else unidata.unicode_grapheme_cluster_break
    )

    if value.startswith("^"):
        negated = value[1:]
        value = "^" + unidata.unicode_alias["graphemeclusterbreak"].get(
            negated, negated
        )
    else:
...
def get_line_break_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_line_break if mode != MODE_UNICODE else unidata.unicode_line_break
    )
...
def get_sentence_break_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_sentence_break
        if mode != MODE_UNICODE
        else unidata.unicode_sentence_break
    )
...
def get_word_break_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_word_break if mode != MODE_UNICODE else unidata.unicode_word_break
    )
...
def get_hangul_syllable_type_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_hangul_syllable_type
        if mode != MODE_UNICODE
        else unidata.unicode_hangul_syllable_type
    )
...
def get_indic_positional_category_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_indic_positional_category
        if mode != MODE_UNICODE
        else unidata.unicode_indic_positional_category
    )
...
def get_indic_syllabic_category_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_indic_syllabic_category
        if mode != MODE_UNICODE
        else unidata.unicode_indic_syllabic_category
    )

    if value.startswith("^"):
        negated = value[1:]
        value = "^" + unidata.unicode_alias["indicsyllabiccategory"].get(
            negated, negated
        )
    else:
...
def get_decomposition_type_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_decomposition_type
        if mode != MODE_UNICODE
        else unidata.unicode_decomposition_type
    )
...
def get_nfc_quick_check_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_nfc_quick_check
        if mode != MODE_UNICODE
        else unidata.unicode_nfc_quick_check
    )
...
def get_nfd_quick_check_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_nfd_quick_check
        if mode != MODE_UNICODE
        else unidata.unicode_nfd_quick_check
    )
...
def get_nfkc_quick_check_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_nfkc_quick_check
        if mode != MODE_UNICODE
        else unidata.unicode_nfkc_quick_check
    )
...
def get_nfkd_quick_check_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_nfkd_quick_check
        if mode != MODE_UNICODE
        else unidata.unicode_nfkd_quick_check
    )
...
def get_numeric_type_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_numeric_type
        if mode != MODE_UNICODE
        else unidata.unicode_numeric_type
    )
...
def get_numeric_value_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_numeric_values
        if mode != MODE_UNICODE
        else unidata.unicode_numeric_values
    )
...
def get_joining_type_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_joining_type
        if mode != MODE_UNICODE
        else unidata.unicode_joining_type
    )
...
def get_joining_group_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_joining_group
        if mode != MODE_UNICODE
        else unidata.unicode_joining_group
    )
...
def get_script_extension_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_script_extensions
        if mode != MODE_UNICODE
        else unidata.unicode_script_extensions
    )
...
def get_bidi_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_bidi_classes
        if mode != MODE_UNICODE
        else unidata.unicode_bidi_classes
    )
...
def get_bidi_paired_bracket_type_property(value: str, mode: int = MODE_UNICODE) -> str:
    ...
    obj = (
        unidata.ascii_bidi_paired_bracket_type
        if mode != MODE_UNICODE
        else unidata.unicode_bidi_paired_bracket_type
    )

    if value.startswith("^"):
        negated = value[1:]
        value = "^" + unidata.unicode_alias["bidipairedbrackettype"].get(
            negated, negated
        )
    else:
...
def get_unicode_property(
    prop: str, value: str | None = None, mode: int = MODE_UNICODE
) -> str:
    ...
    if value is not None:
        ...
        if _is_binary(prop):
            ...
            if value in ("n", "no", "f", "false"):
                ...
                raise ValueError(
                    f"'{value}' is not a valid value for the binary property '{prop}'"
                )
        ...
        try:
            if name == "generalcategory":
                ...
                raise ValueError(
                    f"'{prop}={value}' does not have a valid property name"
                )
        except Exception as e:
            raise ValueError(
                f"'{prop}={value}' does not appear to be a valid property"
            ) from e
