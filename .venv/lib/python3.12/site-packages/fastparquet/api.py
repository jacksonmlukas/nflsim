...
class ParquetFile(object):
    ...
    def __init__(
        ...
        if isinstance(fn, (tuple, list)):
            ...
            basepath, fmd = metadata_from_many(
                fn, verify_schema=verify, open_with=open_with, root=root, fs=fs
            )
            ...
            if self.file_scheme not in ["simple", "empty"]:
                raise ValueError("Cannot use file-like input " "with multi-file data")
            ...
            if fs.isfile(fn):
                ...
                if fs.exists(fn2):
                    ...
                    writer.consolidate_categories(fmd)
                    self.fn = (
                        join_path(basepath, "_metadata") if basepath else "_metadata"
                    )
                    self.fmd = fmd
                    self._set_attrs()
                self.fs = fs
    ...
    def _set_attrs(self):
        ...
        self.selfmade = (
            b"fastparquet" in self.created_by if self.created_by is not None else False
        )
    ...
    def key_value_metadata(self):
        if self._kvm is None:
            self._kvm = {
                ensure_str(k.key, ignore_error=True): ensure_str(
                    k.value, ignore_error=True
                )
    ...
    def partition_meta(self):
        return {
            col["field_name"]: col
            for col in self.pandas_metadata.get("partition_columns", [])
        }
        ...
        new_pf.__setstate__(
            {
                ...
                "_base_dtype": self._base_dtype,
                "tz": self.tz,
                "_columns_dtype": self._columns_dtype,
            }
        )
        new_pf._set_attrs()
        return new_pf
        ...
        if assign is None:
            if row_filter and isinstance(row_filter, list):
                ...
                df = self.read_row_group_file(
                    rg, cs, categories, index=False, infile=infile, row_filter=False
                )
                ...
                size = rg.num_rows
            df, assign = self.pre_allocate(size, columns, categories, index)
    ...
    def remove_row_groups(
        self,
        rgs,
        sort_pnames: bool = False,
        write_fmd: bool = True,
        open_with=default_open,
        remove_with=None,
    ):
        ...
        if rgs:
            if self.file_scheme == "simple":
                raise ValueError(
                    "Not possible to remove row groups when file " "scheme is 'simple'."
                )
    ...
    def write_row_groups(
        ...
        if self.file_scheme == "simple" or (
            self.file_scheme == "empty" and self.fn[-9:] != "_metadata"
        ):
            # Case 'simple'.
            write_simple(
                self.fn,
    ...
    def _sort_part_names(self, write_fmd: bool = True, open_with=default_open):
        ...
        if pids:
            ...
            # name id.
            pids = dict(filter(lambda item: item[0] != item[1][0], pids.items()))
            basepath = self.basepath
    ...
    def _write_common_metadata(self, open_with=default_open):
        ...
        if self.file_scheme == "simple":
            raise ValueError(
                "Not possible to write common metadata when file \
scheme is 'simple'."
            )
        fmd = self.fmd
    ...
    def _columns_from_filters(self, filters):
        return [
            ...
            for c in set(
                sum(
                    [
                        [f[0]] if isinstance(f[0], str) else [g[0] for g in f]
                        for f in filters
                    ],
                    [],
                )
    ...
    def to_pandas(
        ...
        if row_filter is not False:
            if filters and row_filter is True:
                ...
                df = self.to_pandas(
                    columns=cs, filters=filters, row_filter=False, index=False
                )
                ...
                if sum(rg.num_rows for rg in rgs) != len(row_filter):
                    raise ValueError(
                        "Provided boolean array for custom row \
selection does not match number of rows in DataFrame."
        ...
        for rg, sel in zip(rgs, selected):
            ...
            if thislen == rg.num_rows:
                ...
                continue
            parts = {
                name: (v if name.endswith("-catdef") else v[start : start + thislen])
                for (name, v) in views.items()
            }
            self.read_row_group_file(
                rg,
                columns,
                categories,
                index,
    ...
    def pre_allocate(self, size, columns, categories, index, dtypes=None):
        ...
        df, arrs = _pre_allocate(
            ...
            dtypes,
            self.tz,
            columns_dtype=self._columns_dtype,
        )
        ...
        if self.has_pandas_metadata:
            ...
            if md.get("index_columns", False) and not (index or index is False):
                ...
                names = [None if n is None or i_no_name.match(n) else n for n in names]
    ...
    def check_categories(self, cats):
        ...
        if set(cats) - set(categ) and len(self.row_groups) > 1:
            raise TypeError(
                "Attempt to read as category a field that " "was not stored as such"
            )
                    ...
                    for col in rg.columns:
                        ...
                        if col.meta_data.encoding_stats:
                            if any(
                                ...
                                if s.page_type
                                in [
                                    parquet_thrift.PageType.DATA_PAGE_V2,
    ...
    def _dtypes(self, categories=None):
        ...
        if self._base_dtype is None:
            if self.has_pandas_metadata:
                ...
                tz = {
                    k: v["metadata"]["timezone"]
                    for k, v in md.items()
                    if v.get("metadata", {})
                    and v.get("metadata", {}).get("timezone", None)
                }
            else:
                tz = None
                md = None
            ...
            dtype = OrderedDict(
                (
                    name,
                    (
                        converted_types.typemap(f, md=md)
                        if f.num_children in [None, 0]
                        else np.dtype("O")
                    ),
                )
...
def _pre_allocate(
    size, columns, categories, index, cs, dt, tz=None, columns_dtype=None
):
...
def paths_to_cats(paths, partition_meta=None):
    ...
    try:
        ...
        return "drill", _path_to_cats(
            paths, parts, "drill", partition_meta=partition_meta
        )
...
def _path_to_cats(paths, parts, file_scheme="hive", partition_meta=None):
    ...
    for path, path_parts in zip(paths, parts):
        ...
        if file_scheme == "hive":
            hivehits = [
                p.split("=") for p in path.split("/") if "=" in p
            ]  # s.findall(path)
            if not hivehits:
        ...
        for key, val in hivehits:
            ...
            seen.add((key, val))
            tp = val_to_num(
                val, meta if key in string_types else partition_meta.get(key)
            )
...
def statistics(obj):
        ...
        if s.max is not None:
            try:
                if md.type == parquet_thrift.Type.BYTE_ARRAY:
                    ...
                    rv["max"] = encoding.read_plain(
                        ensure_bytes(s.max), md.type, 1, stat=True
                    )[0]
            except:
                rv["max"] = None
            ...
            try:
                if md.type == parquet_thrift.Type.BYTE_ARRAY:
                    ...
                    rv["max"] = encoding.read_plain(
                        ensure_bytes(s.max_value), md.type, 1, stat=True
                    )[0]
            except:
                rv["max"] = None
        if s.min is not None:
            try:
                if md.type == parquet_thrift.Type.BYTE_ARRAY:
                    ...
                    rv["min"] = encoding.read_plain(
                        ensure_bytes(s.min), md.type, 1, stat=True
                    )[0]
            except:
                rv["min"] = None
            ...
            try:
                if md.type == parquet_thrift.Type.BYTE_ARRAY:
                    ...
                    rv["min"] = encoding.read_plain(
                        ensure_bytes(s.min_value), md.type, 1, stat=True
                    )[0]
            except:
                rv["min"] = None
    ...
    if isinstance(obj, ThriftObject) and obj.thrift_name == "RowGroup":
        return {
            ".".join(c.meta_data.path_in_schema): statistics(c) for c in obj.columns
        }
    ...
    if isinstance(obj, ParquetFile):
        ...
        d = {
            n: {
                col: [item.get(col, {}).get(n, None) for item in L]
        ...
        for col in obj.row_groups[0].columns:
            ...
            if (
                se.converted_type is not None
                or se.logicalType is not None
                ...
                for name in ["min", "max"]:
                    try:
                        d[name][column] = (
                            [None]
                            if d[name][column] is None or None in d[name][column]
                            else list(
    ...
    for c in columns:
        ...
        try:
            if (
                ...
                and sorted(min) == min
                and sorted(max) == max
                and all(mx < mn for mx, mn in zip(max[:-1], min[1:]))
            ):
...
def filter_row_groups(pf, filters, as_idx: bool = False):
    ...
    if not all(known):
        ...
        raise ValueError(
            "No filter can be applied on nonexistent column(s) \
{!s}.".format(
                wrong_cols
            )
        )
    if as_idx:
        return [
            ...
            if any(
                [
                    not (filter_out_stats(rg, and_filters, pf.schema))
                    and not (filter_out_cats(rg, and_filters, pf.partition_meta))
    ...
    for cat, v in pairs:
        ...
        for op, val in app_filters:
            if isinstance(val, str) or (
                isinstance(val, (tuple, list)) and all(isinstance(x, str) for x in val)
            ):
                v0 = v
            else:
                v0 = val_to_num(v)
...
def partitions(row_group, only_values=False) -> str:
    ...
    str
        Partitions values.
    """
    f_path = row_group if isinstance(row_group, str) else row_group.columns[0].file_path
    if "/" in f_path:
        return (
            "/".join(re.split("/|=", f_path)[1::2])
            if only_values
...
def part_ids(row_groups) -> dict:
    ...
    return {
        int(pid_path[0]["i"]): (max_rgidx - i, pid_path[1])
