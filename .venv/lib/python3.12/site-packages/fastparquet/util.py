...
def check_column_names(columns, *args):
    ...
    for arg in args:
        if isinstance(arg, (tuple, list)):
            ...
            if missing:
                raise ValueError(
                    "Following columns were requested but are "
                    "not available: %s.\n"
...
def reset_row_idx(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(data.index, pd.MultiIndex):
        for name, cats, codes in zip(
            data.index.names, data.index.levels, data.index.codes
        ):
...
def metadata_from_many(
    file_list, verify_schema=False, open_with=default_open, root=False, fs=None
):
    """
    ...
    if all(isinstance(pf, api.ParquetFile) for pf in file_list):
        ...
        if verify_schema or fs is None or len(file_list) < 3:
            ...
            if pf0.file_scheme not in ["empty", "simple"]:
                ...
                pfs = [pf0] + [
                    api.ParquetFile(fn, open_with=open_with) for fn in file_list[1:]
                ]
            else:
                ...
                sizes = {
                    path: int.from_bytes(piece[-8:-4], "little") + 8
                    ...
                    for chunk in rg.columns:
                        chunk.file_path = "/".join(
                            [
                                ...
                                (
                                    chunk.file_path
                                    if isinstance(chunk.file_path, str)
                                    else chunk.file_path.decode()
                                ),
                            ]
                        )
                    rgs.append(rg)
    ...
    for key, value in custom_metadata.items():
        ...
        if key_b in kvm_keys:
            ...
            if value is None:
                ...
                # Replace item.
                kvm[idx] = parquet_thrift.KeyValue(key=key_b, value=ensure_bytes(value))
        elif value is not None:
            kvm.append(parquet_thrift.KeyValue(key=key_b, value=ensure_bytes(value)))
    if isinstance(obj, ThriftObject):
...
def analyse_paths(file_list, root=False):
    ...
    for path_parts in path_parts_list:
        out_list.append(
            "/".join(path_parts[l:])
        )  # use '/'.join() instead of join_path to be consistent with split('/')
...
def get_column_metadata(column, name, object_dtype=None):
    ...
    if isinstance(dtype, pd.CategoricalDtype):
        ...
        if isinstance(dtype.tz, zoneinfo.ZoneInfo):
            ...
            try:
                ...
                raise ValueError(
                    "Time-zone information could not be serialised: "
    ...
    return {
        ...
        "pandas_type": {
            "string": "unicode",
            "datetime64": ("datetimetz" if hasattr(dtype, "tz") else "datetime"),
            "integer": str(dtype),
            "floating": str(dtype),
