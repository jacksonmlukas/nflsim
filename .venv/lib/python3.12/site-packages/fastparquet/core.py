...
def _read_page(file_obj, page_header, column_metadata):
    ...
    if column_metadata.codec:
        assert (
            len(raw_bytes) == page_header.uncompressed_page_size
...
def read_def(io_obj, daph, helper, metadata, out=None):
    ...
    if not helper.is_required(metadata.path_in_schema):
        max_definition_level = helper.max_definition_level(metadata.path_in_schema)
        ...
        if bit_width:
            ...
            definition_levels = read_data(
                io_obj, parquet_thrift.Encoding.RLE, daph.num_values, bit_width, out=out
            )
        if False and (
            ...
            num_nulls = (
                daph.num_values - (definition_levels == max_definition_level).sum()
            )
        if num_nulls == 0:
            definition_levels = None
...
def read_rep(io_obj, daph, helper, metadata, out=None):
    ...
    if len(metadata.path_in_schema) > 1:
        max_repetition_level = helper.max_repetition_level(metadata.path_in_schema)
        if max_repetition_level == 0:
            repetition_levels = None
        else:
            bit_width = encoding.width_from_max_int(max_repetition_level)
            # NB: num_values is index 1 for either type of page header
            repetition_levels = read_data(
                io_obj, parquet_thrift.Encoding.RLE, daph.num_values, bit_width, out=out
            )
    return repetition_levels


def read_data_page(f, helper, header, metadata, skip_nulls=False, selfmade=False):
    ...
    if daph.encoding == parquet_thrift.Encoding.PLAIN:
        ...
        if bit_width in [8, 16, 32] and selfmade:
            ...
            values = np.frombuffer(
                io_obj.read(num * bit_width // 8), dtype="int%i" % bit_width
            )
        elif bit_width:
            if bit_width > 8:
...
def read_dictionary_page(
    file_obj, schema_helper, page_header, column_metadata, utf=False
):
    ...
    if column_metadata.type == parquet_thrift.Type.BYTE_ARRAY:
        ...
        width = schema_helper.schema_element(column_metadata.path_in_schema).type_length
        values = read_plain(
            raw_bytes,
...
def read_data_page_v2(
    ...
    """
    if data_header2.encoding not in [
        parquet_thrift.Encoding.PLAIN_DICTIONARY,
        parquet_thrift.Encoding.RLE_DICTIONARY,
        parquet_thrift.Encoding.RLE,
    ...
    if max_rep:
        ...
        io_obj = encoding.NumpyIO(
            infile.read(data_header2.repetition_levels_byte_length)
        )
        repi = np.empty(data_header2.num_values, dtype="uint8")
        encoding.read_rle_bit_packed_hybrid(
            io_obj,
            bit_width,
            data_header2.num_values,
            encoding.NumpyIO(repi),
            itemsize=1,
        )
    ...
    if max_def and data_header2.num_nulls:
        ...
        io_obj = encoding.NumpyIO(
            infile.read(data_header2.definition_levels_byte_length)
        )
        if nullable:
            defi = assign._mask
        else:
    ...
    see = (
        se.type_length == assign.dtype.itemsize * 8
        or simple.get(se.type).itemsize == assign.dtype.itemsize
    )
    # can read-into
    into0 = (
        (use_cat or converts_inplace(se) and see)
        and data_header2.num_nulls == 0
        and max_rep == 0
        and assign.dtype.kind != "O"
        and row_filter is None
        and assign.dtype.kind not in "Mm"
    )  # TODO: this can be done in place but is complex
    if row_filter is None:
        row_filter = Ellipsis
    # can decompress-into
    if data_header2.is_compressed is None:
        data_header2.is_compressed = True
    into = data_header2.is_compressed and rev_map[cmd.codec] in decom_into and into0
    if nullable:
        assign = assign._data
    ...
    if (
        into0
        and data_header2.encoding == parquet_thrift.Encoding.PLAIN
        and (
            not data_header2.is_compressed
            or cmd.codec == parquet_thrift.CompressionCodec.UNCOMPRESSED
        )
    ):
        ...
        decomp(
            np.frombuffer(infile.read(size), dtype="uint8"),
        ...
        raw_bytes = decompress_data(
            np.frombuffer(infile.read(size), "uint8"), uncompressed_page_size, codec
        )
        values = read_plain(
            raw_bytes,
            cmd.type,
        ...
        if data_header2.num_nulls:
            if nullable:
                assign[num : num + data_header2.num_values][~nulls[row_filter]] = (
                    convert(values, se)[row_filter]
                )
            else:
                assign[num : num + data_header2.num_values][
                    nulls[row_filter]
                ] = None  # or nan or nat
                if row_filter is Ellipsis:
                    assign[num : num + data_header2.num_values][~nulls] = convert(
                        values, se
                    )
                else:
                    assign[num : num + data_header2.num_values][~nulls[row_filter]] = (
                        convert(values, se)[row_filter[~nulls]]
                    )
        else:
        ...
        if bit_width in [8, 16, 32] and selfmade:
            ...
            if len(outbytes) == assign[num : num + data_header2.num_values].nbytes:
                ...
                if data_header2.num_nulls == 0:
                    assign[num : num + data_header2.num_values][row_filter] = outbytes[
                        row_filter
                    ]
                else:
                    if row_filter is Ellipsis:
            ...
            if data_header2.num_nulls == 0:
                ...
                assign[num : num + data_header2.num_values][~nulls[row_filter]] = temp[
                    row_filter
                ]
        ...
        if max_rep:
            ...
            encoding._assemble_objects(
                assign[idx[0] : idx[0] + data_header2.num_rows],
                defi,
                repi,
                out,
                dic,
            ...
            if not nullable and assign.dtype != "O":
                assign[num : num + data_header2.num_values][
                    nulls
                ] = None  # may be unnecessary
            assign[num : num + data_header2.num_values][~nulls[row_filter]] = dic[out][
                row_filter
            ]
        else:
            assign[num : num + data_header2.num_values][row_filter] = dic[out][
                row_filter
            ]
        ...
        if converts_inplace(se):
            ...
            assign[num : num + data_header2.num_values][row_filter] = convert(out, se)[
                row_filter
            ]
    else:
        # codec = cmd.codec if data_header2.is_compressed else "UNCOMPRESSED"
...
def read_col(
    ...
    try:
        ...
        return
    off = min(
        (cmd.dictionary_page_offset or cmd.data_page_offset, cmd.data_page_offset)
    )
    ...
    while num < rows:
        ...
        if ph.type == parquet_thrift.PageType.DICTIONARY_PAGE:
            dic2 = read_dictionary_page(
                infile, schema_helper, ph, cmd, utf=se.converted_type == 0
            )
            dic2 = convert(dic2, se)
            if use_cat and dic is not None and (dic2 != dic).any():
                raise RuntimeError(
                    "Attempt to read as categorical a column"
            ...
            if use_cat and dic is not None:
                ...
                ddt = [
                    kv.value.decode()
                    for kv in (cmd.key_value_metadata or [])
            ...
            raise TypeError(
                "Attempt to load as categorical a column with no dictionary"
            )
        ...
        if (
            selfmade
            and hasattr(cmd, "statistics")
            and getattr(cmd.statistics, "null_count", 1) == 0
        ):
            skip_nulls = True
        else:
            skip_nulls = False
        defi, rep, val = read_data_page(
            infile, schema_helper, ph, cmd, skip_nulls, selfmade=selfmade
        )
        ...
        if isinstance(row_filter, np.ndarray):
            ...
            if defi is not None:
                val = val[
                    row_filter[index_off : index_off + len(defi)][defi == max_defi]
                ]
                defi = defi[row_filter[index_off : index_off + len(defi)]]
            else:
                val = val[row_filter[index_off : index_off + len(val)]]
            rep = (
                rep[row_filter[index_off : index_off + len(defi)]]
                if rep is not None
                else rep
            )
            index_off = io
        if rep is not None and assign.dtype.kind != "O":  # pragma: no cover
            ...
            raise ValueError(
                "Column contains repeated value, must use object "
        ...
        if use_cat and not d:
            if not hasattr(catdef, "_set_categories"):
                raise ValueError(
                    "Returning category type requires all chunks"
...
def read_row_group_arrays(
    ...
    out = assign
    remains = set(
        _ for _ in out if not _.endswith("-catdef") and not _ + "-catdef" in out
    )
    maps = {}

    for column in rg.columns:

        if _is_list_like(schema_helper, column) or _is_map_like(schema_helper, column):
        ...
        if _is_map_like(schema_helper, column):
            ...
            if name not in maps:
                ...
                out[name][:] = [
                    dict(zip(k, v)) if k is not None else None
...
def read_row_group(
    ...
    for cat in cats:
        ...
        if scheme == "hive":
            ...
            partitions = [
                ("dir%i" % i, v)
                for (i, v) in enumerate(rg.columns[0].file_path.split("/")[:-1])
            ]
