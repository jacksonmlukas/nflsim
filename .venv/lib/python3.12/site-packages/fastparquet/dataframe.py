...
def empty(
        ...
        if str(t) == "category":
            ...
            if d.dtype.kind == "M" and str(col) in timezones:
                try:
                    ...
                    warnings.warn(
                        "Inferring time-zone from %s in column %s "
    ...
    columns = (
        Index(df.keys(), dtype=columns_dtype) if columns_dtype is not None else None
    )
        ...
        for i, col in enumerate(index_names):
            ...
            def set_cats(values, i=i, col=col, **kwargs):
                ...
                if index._levels[i][0] is None:
                    ...
                    raise RuntimeError(
                        "Different dictionaries encountered"
    ...
    for block in mgr.blocks:
        ...
        if isinstance(bvalues, Categorical):
            ...
            values = type(bvalues)._from_sequence(
                values.view("int64"), copy=False, dtype=bvalues.dtype
            )
        else:
            if not isinstance(bvalues, np.ndarray):
                ...
                if bvalues.dtype.kind == "m":
                    ...
                    str(bvalues.dtype)[0] in {"I", "U"}
                    or str(bvalues.dtype) == "boolean"
                ):
    ...
    if index_names:
        df.index.names = [
            None if re.match(r"__index_level_\d+__", n) else n for n in index_names
        ]
    return df, views
