...
def find_type(
    data, fixed_text=None, object_encoding=None, times="int64", is_index: bool = None
):
    ...
    if dtype.name in typemap:
        ...
        if object_encoding == "utf8":
            type, converted_type, width = (
                parquet_thrift.Type.BYTE_ARRAY,
            ...
            type, converted_type, width = (parquet_thrift.Type.BOOLEAN, None, 1)
        elif object_encoding == "int":
            type, converted_type, width = (parquet_thrift.Type.INT64, None, 64)
        elif object_encoding == "int32":
            type, converted_type, width = (parquet_thrift.Type.INT32, None, 32)
        elif object_encoding == "float":
            type, converted_type, width = (parquet_thrift.Type.DOUBLE, None, 64)
        elif object_encoding == "decimal":
            type, converted_type, width = (parquet_thrift.Type.DOUBLE, None, 64)
        else:
            raise ValueError(
                "Object encoding (%s) not one of "
        ...
        if times == "int64":
            ...
            if "ns" in dtype.str:
                ...
                logical_type = parquet_thrift.LogicalType(
                    TIMESTAMP=parquet_thrift.TimestampType(
                        isAdjustedToUTC=tz,
                        unit=parquet_thrift.TimeUnit(
                            NANOS=parquet_thrift.NanoSeconds()
                        ),
                    )
                )
                width = None
            ...
            type, converted_type, width = (parquet_thrift.Type.INT96, None, None)
        else:
            raise ValueError("Parameter times must be [int64|int96], not %s" % times)
    ...
    se = parquet_thrift.SchemaElement(
        name=norm_col_name(data.name, is_index),
        type_length=width,
...
def convert(data, se):
    ...
    if dtype.name in typemap:
        ...
        try:
            ...
            ct = (
                parquet_thrift.ConvertedType._VALUES_TO_NAMES[converted_type]
                if converted_type is not None
                else None
            )
            raise ValueError(
                'Error converting column "%s" to bytes using '
        ...
        if converted_type:
            ...
            unit = [
                k
                for k, v in se.logicalType.TIMESTAMP.unit._asdict().items()
                if v is not None
            ][0]
...
def time_shift(indata, outdata, factor=1000):
    outdata.view("int64")[:] = np.where(
        indata.view("int64") == nat, nat, indata.view("int64") // factor
    )
...
def _rows_per_page(data, selement, has_nulls=True, page_size=None):
    ...
    if isinstance(data.dtype, pd.CategoricalDtype):
        ...
        isinstance(data.dtype, BaseMaskedDtype)
        and data.dtype in pdoptional_to_numpy_typemap
    ):
        ...
        try:
            ...
            bytes_per_element = (
                dd.memory_usage(index=False, deep=True) / (len(dd) or 1)
            ) or 16
    else:
...
def write_column(
    f, data0, selement, compression=None, datapage_version=None, stats=True
):
    """
        ...
        try:
            ...
            if pd.isna(max):
                ...
                if selement.type == parquet_thrift.Type.BYTE_ARRAY:
                    ...
                    max = encode["PLAIN"](
                        pd.Series([max], name=name, dtype=data0.dtype), selement
                    )
                    min = encode["PLAIN"](
                        pd.Series([min], name=name, dtype=data0.dtype), selement
                    )
        except (TypeError, ValueError):
            stats = False
    ...
    for row_start, row_end in zip(row_offsets[:-1], row_offsets[1:]):
        ...
        if has_nulls:
            ...
            definition_data, data = make_definitions(
                data, num_nulls == 0, datapage_version=datapage_version
            )
            ...
            if data.dtype.kind == "O" and not isinstance(
                ...
                try:
                    ...
                    raise ValueError(
                        'Error converting column "%s" to primitive '
    ...
    cmd = ThriftObject.from_fields(
        ...
        encodings=encodings,
        codec=(
            getattr(parquet_thrift.CompressionCodec, algorithm.upper())
            if algorithm
            else 0
        ),
        num_values=tot_rows,
    ...
    if cats:
        kvm.append(parquet_thrift.KeyValue(key="num_categories", value=str(ncats)))
        kvm.append(parquet_thrift.KeyValue(key="numpy_dtype", value=str(data.dtype)))
        kvm.append(parquet_thrift.KeyValue(key="label_dtype", value=str(dcat)))
    chunk = parquet_thrift.ColumnChunk(
        file_offset=column_chunk_start, meta_data=cmd, file_path=None
    )
    return chunk
...
def make_row_group(f, data, schema, compression=None, stats=True):
    ...
    if isinstance(data.columns, pd.MultiIndex):
        if any(
            not isinstance(c, (bytes, str))
            for c in itertools.chain(*data.columns.values)
        ):
            raise ValueError(
                "Column names must be multi-index, str or bytes:",
                {c: type(c) for c in data.columns if not isinstance(c, (bytes, str))},
            )
    ...
    for column in schema:
        if column.type is not None:
            ...
            chunk = write_column(f, coldata, column, compression=comp, stats=st)
            cols.append(chunk)
...
def make_metadata(
    ...
    if not data.columns.is_unique:
        raise ValueError(
            "Cannot create parquet dataset with duplicate"
    ...
    if isinstance(data.columns, pd.MultiIndex):
        ...
        ci = [
            ...
            for ser, n in zip(data.columns.levels, data.columns.names)
        ]
    else:
        ci = [
            {
    ...
    pandas_metadata = {
        ...
        "columns": [],
        "column_indexes": ci,
        "creator": {"library": "fastparquet", "version": __version__},
        "pandas_version": pd.__version__,
    }
    root = parquet_thrift.SchemaElement(name=b"schema", num_children=0, i32=True)
    meta = parquet_thrift.KeyValue(key=b"pandas", value=None)
    fmd = ThriftObject.from_fields(
        "FileMetaData",
        num_rows=len(data),
        schema=None,
        version=1,
    ...
    for column in partition_cols:
        pandas_metadata["partition_columns"].append(
    ...
    for column in data.columns:
        ...
        oencoding = (
            object_encoding
            if isinstance(object_encoding, str)
...
def write_simple(
    ...
    def write_to_file(f):
        ...
        for i, row_group in enumerate(data):
            rg = make_row_group(
                f, row_group, fmd.schema, compression=compression, stats=stats
            )
            if rg is not None:
                rgs.append(rg)
...
def write_multi(
    ...
    if write_fmd:
        write_common_metadata(
            join_path(dn, "_metadata"), fmd, open_with, no_row_groups=False
        )
        write_common_metadata(join_path(dn, "_common_metadata"), fmd, open_with)
...
def write(
    ...
    if file_scheme not in ("simple", "hive", "drill"):
        raise ValueError(
            "File scheme should be simple|hive|drill, not " f"{file_scheme}."
        )
    ...
    if append:
        ...
        if file_scheme == "simple":
            ...
            if pf.file_scheme not in ["simple", "empty"]:
                raise ValueError(
                    "File scheme requested is simple, but "
            ...
            if pf.file_scheme not in ["hive", "empty", "flat"]:
                raise ValueError(
                    f"Requested file scheme is {file_scheme}, but"
                    " existing file scheme is not."
                )
            if tuple(partition_on) != tuple(pf.cats):
                raise ValueError(
                    "When appending, partitioning columns must " "match existing data"
                )
        pf.write_row_groups(
            data,
            row_group_offsets,
        ...
        if str(has_nulls) == "infer":
            has_nulls = None
        check_column_names(
            data.columns, partition_on, fixed_text, object_encoding, has_nulls
        )
...
def partition_on_columns(
    ...
    for key, group in sorted(gb):
        ...
        if with_field:
            path = join_path(
                *("%s=%s" % (name, path_string(val)) for name, val in zip(columns, key))
            )
        else:
        ...
        with open_with(fullname, "wb") as f2:
            rg = make_part_file(
                f2, df, fmd.schema, compression=compression, fmd=fmd, stats=stats
            )
        if rg is not None:
            for chunk in rg.columns:
...
def write_common_metadata(fn, fmd, open_with=default_open, no_row_groups=True):
    """
...
def consolidate_categories(fmd):
    key_value = [k for k in fmd.key_value_metadata or [] if k.key == b"pandas"]
    if not key_value:
        # no pandas categories
        return
    key_value = key_value[0]
    meta = json.loads(key_value.value)
    cats = [c for c in meta["columns"] if "num_categories" in (c["metadata"] or [])]
    for cat in cats:
        for rg in fmd.row_groups:
            for col in rg.columns:
                if ".".join(col.meta_data.path_in_schema) == cat["name"]:
                    ncats = [
                        k.value
                        for k in (col.meta_data.key_value_metadata or [])
                        if k.key == b"num_categories"
                    ]
                    if ncats and int(ncats[0]) > cat["metadata"]["num_categories"]:
...
def overwrite(
    ...
    if pf.file_scheme == "simple" or (
        pf.file_scheme == "empty" and pf.fn[-9:] != "_metadata"
    ):
        raise ValueError("Not possible to overwrite with simple file " "scheme.")
    defined_partitions = list(pf.cats)
    if not defined_partitions:
        raise ValueError(
            "No partitioning column has been set in existing "
    ...
    max_idx = n_rgs - 1
    partitions_starts = {
        partitions(rg): (max_idx - i) for i, rg in enumerate(reversed(pf.row_groups))
    }
    ...
    def sort_key(row_group) -> int:
        ...
        return (
            partitions_starts[rg_partition]
            if (rg_partition in partitions_starts)
            else n_rgs
        )
    ...
    # those in new data.
    partition_values_in_new = pd.unique(
        data.loc[:, defined_partitions].astype(str).agg("/".join, axis=1)
    )
    rgs_to_remove = filter(
        lambda rg: (partitions(rg, True) in partition_values_in_new), pf.row_groups
    )
    ...
    if obj.thrift_name == "FileMetaData":
        for kv in obj.key_value_metadata:
            if not isinstance(kv.key, (bytes, str)):
                raise TypeError(
                    f"KeyValue key expected `str` or `bytes`, got: {kv.key!r}"
                )
            if not isinstance(kv.value, (bytes, str)):
                raise TypeError(
                    f"KeyValue value expected `str` or `bytes`, got: {kv.value!r}"
                )
    return f.write(obj.to_bytes())


def update_file_custom_metadata(
    path: str, custom_metadata: dict, is_metadata_file: bool = None
):
