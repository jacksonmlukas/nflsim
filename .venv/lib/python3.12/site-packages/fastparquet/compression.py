...
compressions = {"UNCOMPRESSED": lambda x: x}
decompressions = {"UNCOMPRESSED": lambda x, y: x}
...
rev_map = {
    getattr(parquet_thrift.CompressionCodec, key): key
    for key in dir(parquet_thrift.CompressionCodec)
    if key
    in ["UNCOMPRESSED", "SNAPPY", "GZIP", "LZO", "BROTLI", "LZ4", "ZSTD", "LZ4_RAW"]
}
...
def compress_data(data, compression="gzip"):
    ...
    if algorithm.upper() not in compressions:
        raise RuntimeError(
            "Compression '%s' not available.  Options: %s"
