...
def schema_to_text(root, indent=[]):
        ...
        for key in dir(root.logicalType):
            if getattr(root.logicalType, key) is not None:
                if key == "TIMESTAMP":
                    unit = [
                        k
                        for k in dir(root.logicalType.TIMESTAMP.unit)
                        if getattr(root.logicalType.TIMESTAMP.unit, k) is not None
                    ][0]
    ...
    if root.converted_type is not None:
        parts.append(parquet_thrift.ConvertedType._VALUES_TO_NAMES[root.converted_type])
    if root.repetition_type is not None:
        parts.append(
            parquet_thrift.FieldRepetitionType._VALUES_TO_NAMES[root.repetition_type]
        )
    text += ", ".join(parts)
    indent.append("|")
...
def flatten(schema, root, name_parts=[]):
    ...
    for name, item in schema["children"].copy().items():
        ...
        elif item.converted_type in [
            parquet_thrift.ConvertedType.LIST,
...
class SchemaHelper(object):
    ...
    def __repr__(self):
        return "<Parquet Schema with {} entries>".format(len(self.schema_elements))
...
def _is_list_like(helper, column):
    if len(column.meta_data.path_in_schema) < 3:
        return False
    se = helper.schema_element(column.meta_data.path_in_schema[:-2])
