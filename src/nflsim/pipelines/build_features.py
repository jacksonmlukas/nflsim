...
OUT_ROOT = Path("data/processed/features/offense_team_week")

# Use a raw triple-quoted Python string so backslashes in regex stay intact
SQL = r"""
WITH j AS (
  SELECT
    p.season,
...
agg AS (
  SELECT
    ...
    posteam AS team,
    COUNT(*) AS plays,
    AVG(CASE WHEN is_pass = 1 THEN 1 ELSE 0 END) AS pass_rate,
    AVG(number_of_pass_rushers) AS avg_rushers,
    AVG(CASE WHEN was_pressure THEN 1 ELSE 0 END) AS pressure_rate,
    AVG(CASE WHEN rb = 1 AND te = 1 AND wr = 3 THEN 1 ELSE 0 END) AS rate_11,
    AVG(CASE WHEN rb = 1 AND te = 2 AND wr = 2 THEN 1 ELSE 0 END) AS rate_12,
    AVG(CASE WHEN rb = 1 AND te = 3 THEN 1 ELSE 0 END) AS rate_13,
    AVG(CASE WHEN rb = 1 AND COALESCE(te, 0) = 0 AND wr = 4 THEN 1 ELSE 0 END) AS rate_10,
    AVG(CASE WHEN mz_type = 'man' OR cov_type = 'man' THEN 1 ELSE 0 END) AS man_rate,
    AVG(CASE WHEN mz_type = 'zone' OR cov_type = 'zone' THEN 1 ELSE 0 END) AS zone_rate
  FROM parsed
  GROUP BY 1, 2, 3
)
SELECT * FROM agg
